...
func testAuthPermissionToManyPermissionAuthGroupPermissions(t *testing.T) {
	...
	authGroupPermission, err := a.PermissionAuthGroupPermissions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}

	bFound, cFound := false, false
	for _, v := range authGroupPermission {
...
func testAuthPermissionToManyPermissionAuthUserUserPermissions(t *testing.T) {
	...
	authUserUserPermission, err := a.PermissionAuthUserUserPermissions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	authPermissionDBTypes = map[string]string{`Codename`: `character varying`, `ContentTypeID`: `integer`, `ID`: `integer`, `Name`: `character varying`}
	_                     = bytes.MinRead
)
