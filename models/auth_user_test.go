...
func testAuthUserToManyUserAuthUserGroups(t *testing.T) {
	...
	authUserGroup, err := a.UserAuthUserGroups().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}

	bFound, cFound := false, false
	for _, v := range authUserGroup {
...
func testAuthUserToManyUserAuthUserUserPermissions(t *testing.T) {
	...
	authUserUserPermission, err := a.UserAuthUserUserPermissions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}

	bFound, cFound := false, false
	for _, v := range authUserUserPermission {
		if v.UserID == b.UserID {
			bFound = true
		}
		if v.UserID == c.UserID {
	...
	if t.Failed() {
		t.Logf("%#v", authUserUserPermission)
	}
}
...
var (
	authUserDBTypes = map[string]string{`DateJoined`: `timestamp with time zone`, `Email`: `character varying`, `FirstName`: `character varying`, `ID`: `integer`, `IsActive`: `boolean`, `IsStaff`: `boolean`, `IsSuperuser`: `boolean`, `LastLogin`: `timestamp with time zone`, `LastName`: `character varying`, `Password`: `character varying`, `Username`: `character varying`}
	_               = bytes.MinRead
)
